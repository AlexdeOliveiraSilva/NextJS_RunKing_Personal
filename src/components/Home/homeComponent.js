...
import FormMedicalRequest from "../FormMedicalRequest";
...
export default function HomeComponent({ path }) {
  ...
  return (
    <main className="fullContainer">
      {modalCapture == true && (
        <ModalTakePic close={() => closeAndSave()}></ModalTakePic>
      )}
      ...
      <div className="homeContent">
        <div className="userContent">
          <ProfileContent
            ...
            name={!!image ? "Rafael Oliveira" : "-"}
            number={!!image ? "10015441" : "-"}
          ></ProfileContent>
          <TakePicture
            status={!!image ? true : false}
            img={image}
        ...
        {!image ? (
          ...
          <button onClick={() => signOut()} className="btnRed profileBtn">
            {isLoading == true ? <Loading></Loading> : "Deslogar"}
          </button>
        )}
      </div>
      <Footer></Footer>
